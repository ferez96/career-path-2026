...
6. Score gap/fit vs personal profile (resume + target metadata in `docs/cursor/fit-weights.md`).
...
**Reports with company names / sensitive detail:** default to `data/reports/pipeline/`, `data/reports/roles/`, and related grouped folders under `data/reports/`. Public-safe excerpts → `data/reports/briefs/` after `docs/SANITIZATION_CHECKLIST.md`. Optional links: `milestone_id` with `data/career_path_master.csv`, `jd_source` with raw JD / catalog.
