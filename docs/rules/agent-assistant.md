...
- Analysis / fit / gap: follow `docs/rules/career-path-resume.md` (resume via `data/private/master.yaml` when applicable). **`data/**` is a private vault** — full detail is fine there. **Public-bound** output (tracked paths outside `data/` on `master`) must stay non-PII per `docs/SANITIZATION_CHECKLIST.md`.
