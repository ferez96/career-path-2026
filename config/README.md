...
- `context_manifest.yaml`: three-section agent context index:
  ...
  - `contexts:` — individual file entries with path, token_budget, and visibility. Add entries here when you have new **tracked, sanitized** artifacts, then reference the new id in the appropriate group (see `docs/cursor/workflows.md` Opportunity Tracking for pipeline context).
